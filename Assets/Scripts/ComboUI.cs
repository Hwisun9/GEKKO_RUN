...
public class ComboUI : MonoBehaviour
{
    ...
    public GameObject specialComboEffect; // 5콤보 특별 효과 패널
    ...
    void Start()
    {
        ...
        // 초기 UI 상태 설정 (패널은 활성화 상태로 두고 개별 요소만 숨김)
        SetupInitialUI();

        // ComboSystem 연결을 지연시켜 확실하게 연결
        StartCoroutine(DelayedComboSystemConnection());
    }

    void SetupInitialUI()
    {
        // 패널은 활성화 상태로 유지하되, 개별 요소들만 숨김
        if (comboPanel != null)
        {
